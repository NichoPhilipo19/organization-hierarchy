...
export const OrgChart = forwardRef<OrgChartHandle, OrgChartProps>(function OrgChart(
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      exportToPng: (filename = 'org-chart.png') => exportChartToPng(treeRef.current, filename),
    }),
  ...
  const focusById = (id: string) => {
    setFocusedId(id);
    treeRef.current
      ?.querySelector<HTMLElement>(`[data-orgchart-node="${CSS.escape(id)}"]`)
      ?.focus();
  };
  ...
  const onTreeKeyDown = (e: KeyboardEvent<HTMLUListElement>) => {
    ...
    switch (e.key) {
      case 'ArrowDown':
        if (idx >= 0 && idx + 1 < visible.length) focusById(visible[idx + 1]!.node.id);
        break;
      case 'ArrowUp':
      ...
      case 'Home':
        ...
        break;
      case 'End':
        if (visible.length > 0) focusById(visible[visible.length - 1]!.node.id);
        break;
      case 'Enter':
      case ' ':
  ...
  const containerClass = [s.chart, zoomable ? s.chartZoomable : '', className ?? '']
    .filter(Boolean)
    .join(' ');
  ...
  const tree = (
    <ChartProvider value={ctx}>
      ...
      >
        {roots.map((root, i) => (
          <Branch key={root.node.id} tree={root} posInSet={i + 1} setSize={roots.length} />
        ))}
      </ul>
    </ChartProvider>
  );

  return <div className={containerClass}>{zoomable ? <ZoomPane>{tree}</ZoomPane> : tree}</div>;
});
